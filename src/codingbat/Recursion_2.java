...
public class Recursion_2 {
	 ...
	 *
	 * groupSum5(0, {2, 5, 10, 4}, 19) → true groupSum5(0, {2, 5, 10, 4}, 17) →
	 * true groupSum5(0, {2, 5, 10, 4}, 12) → false
	 */
	...
	private boolean helper(int[] nums, int start, int sum1, int sum2) {
		if (start >= nums.length) {
			return sum1 == sum2;
		}
		int value = nums[start];

		if (helper(nums, start + 1, sum1 + value, sum2)) {
			return true;
		}
		if (helper(nums, start + 1, sum1, sum2 + value)) {
			return true;
		}
		return false;
	}
	...
	private boolean helperOdd(int[] nums, int start, int sum1, int sum2) {
		if (start >= nums.length) {
	...
	public boolean split53(int[] nums) {
		return helperSplit53(nums, 0, 0, 0);
	}

	private boolean helperSplit53(int[] nums, int start, int sum1, int sum2) {
		if (start >= nums.length) {
			return sum1 == sum2;
		}
		int value = nums[start];
